...
def test_add_pet_without_name(get_api_key):
    pet_friends = PetFriends(get_api_key)
    status_code, response = pet_friends.add_pet('', 'dog', '4')
    assert status_code == 400

def test_add_pet_with_invalid_age(get_api_key):
    pet_friends = PetFriends(get_api_key)
    status_code, response = pet_friends.add_pet('Jake', 'dog', 'invalid_age')
    assert status_code == 400
...
def test_add_pet_with_photo(get_api_key):
    ...
    with open('path/to/photo.jpg', 'rb') as photo:
        status_code, response = pet_friends.add_pet('Hedwig', 'owl', '2', pet_photo=photo)
    assert status_code == 200
...
def test_add_new_pet_without_photo(get_api_key):
    pet_friends = PetFriends(get_api_key)
    status_code, response = pet_friends.add_new_pet_without_photo('Lesley', 'cat', '3')
    assert status_code == 200
    assert 'id' in response